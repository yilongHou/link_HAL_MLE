...
class ProximalNewtonLBFGSFullEstimator(BaseEstimator):
    ...
    def fit(self, data: pd.DataFrame, warm_start_coefficients: Optional[np.ndarray] = None,
        ...
        print(f"Starting Proximal Newton + L-BFGS with K={K} parameters, memory={self.lbfgs_memory}")
        ...
        for iter_k in range(self.n_iterations):
            ...
            d_newton = self._solve_proximal_subproblem(theta, lbfgs_direction, gamma_k)
    ...
    def get_density(self) -> Tuple[np.ndarray, np.ndarray]:
        """
        Compute the estimated density function on a uniform evaluation grid.
        
        Returns the fitted HAL density estimate:
        f̂(x) = exp(φ(x)ᵀθ̂) / ∫ exp(φ(u)ᵀθ̂) du
        
        where θ̂ are the fitted parameters and φ(x) are the HAL basis functions.
        
        The normalization constant is computed using the same Riemann sum approach
        as during training to ensure consistency.
        
        Returns:
            Tuple[np.ndarray, np.ndarray]
                - grid_points: Evaluation points on [0, 1] (shape: n_grid_points,)
                - density_values: Density estimates f̂(x) at grid points (shape: n_grid_points,)
        ...
        Implementation Details:
        ======================
        - **Evaluation Grid**: Uniform grid on [0, 1] with n_grid_points points
        - **Basis Functions**: Same HAL basis as used in training
        - **Normalization**: Uses midpoint Riemann sum for consistency with training
        - **Numerical Stability**: Uses max subtraction before exponentiation
        - **Final Normalization**: Ensures ∫ f̂(x) dx = 1 via discrete integration
        
        Mathematical Formulation:
        ========================
        The density at evaluation point x is:
        f̂(x) = exp(φ(x)ᵀθ̂ - log Z)
        
        where the normalization constant is:
        Z = ∫ exp(φ(u)ᵀθ̂) du ≈ ∑ⱼ exp(φ(uⱼ)ᵀθ̂) Δⱼ
        
        with {uⱼ} being the midpoints of the integration grid and {Δⱼ} the grid widths.
        
        Properties:
        ===========
        - **Non-negativity**: f̂(x) ≥ 0 for all x (guaranteed by exponential form)
        - **Normalization**: ∫₀¹ f̂(x) dx = 1 (enforced by construction)
        - **Smoothness**: Depends on basis order (step functions vs. splines)
        - **Sparsity**: Non-zero coefficients correspond to selected knots
        ...
        >>> plt.title('HAL Density Estimate')
        >>> plt.xlabel('x')
        >>> plt.ylabel('f(x)')
        """
    ...
    def _compute_lbfgs_direction(self, gradient: np.ndarray, s_list: List[np.ndarray], 
        ...
        effective_gamma = np.clip(gamma_k, 1e-3, 10.0)
    ...
    def _solve_proximal_subproblem(self, theta: np.ndarray, lbfgs_direction: np.ndarray, gamma_k: float) -> np.ndarray:
        """
        ...
        effective_gamma = max(gamma_k, 1e-3)
