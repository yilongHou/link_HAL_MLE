...
class ProximalAdaGradEstimator(BaseEstimator):
    ...
    def fit(self, data: pd.DataFrame, warm_start_coefficients: Optional[np.ndarray] = None,
        ...
        # Convert to numpy arrays - these already include intercept
        ...
        for iter_k in range(self.n_iterations):
            ...
            if iter_k % self.log_frequency == 0 or iter_k < 10:
                ...
                if iter_k % self.log_frequency == 0:
                    print(f"Iter {iter_k:3d}: obj={obj:.6f}, change={change:.2e}, "
                          f"α={self.alpha:.3f}, ‖θ[1:]‖₁={l1_norm:.3f}, h_range=[{h_min:.2e}, {h_max:.2e}], num_knots={num_knots}")
                ...
                if obj_prev is not None:
                    ...
                    if rel_obj_change < self.relative_objective_tolerance and change < self.tol:
                        ...
                        print(f"Converged at iter {iter_k} (relative objective change)")
                        break
                obj_prev = obj
            ...
            if change < self.tol:
                ...
                print(f"Converged at iter {iter_k} (parameter change)")
                break
