...
class CVXPYEstimator:
    ...
    def __init__(
        ...
        Args:
            ...
            solver: CVXPY solver to use (default: "MOSEK" for high reliability)
        """
        self.tol = tol
    ...
    def fit(self, data: pd.DataFrame) -> 'CVXPYEstimator':
        ...
        basis_tensor, basis_names = create_basis_functions(data, grid_points_hal, order=self.basis_order)
        b_ik = basis_tensor.detach().numpy()  # shape: (n_samples, K)
        ...
        basis_grid_tensor, _ = create_basis_functions(data_grid, grid_points_hal, order=self.basis_order)
        b_jk = basis_grid_tensor.numpy()  # shape: (len(grid_midpoints), K)
        ...
        try:
            ...
            print(f"{self.solver} solver failed when using parameters BasisOrder={self.basis_order} and NormConstraint={self.norm_constraint}")
            print("Trying MOSEK as primary solver...")
            try:
                problem.solve(solver="MOSEK")
                print("MOSEK succeeded as primary solver")
            except Exception as e1:
                print(f"MOSEK failed: {e1}")
                print("Falling back to CLARABEL as secondary solver...")
                try:
                    problem.solve(solver="CLARABEL")
                    print("CLARABEL succeeded as secondary solver")
                except Exception as e2:
                    print(f"CLARABEL also failed: {e2}")
                    print("Falling back to ECOS as tertiary solver...")
                    try:
                        problem.solve(solver="ECOS")
                        print("ECOS succeeded as tertiary solver")
                    except Exception as e3:
                        print(f"ECOS also failed: {e3}")
                        print("Final fallback to OSQP...")
        ...
        self.fitted_theta_dict = {name: value for name, value in zip(self.basis_names, self.theta_hat)}
    ...
    def get_density(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        b_jk_full = basis_grid_tensor.numpy()  # Shape: (len(grid_midpoints), K)
        
        # Use the standardized self.theta_hat for calculation
        estimated_log_density = b_jk_full @ self.theta_hat
        
        estimated_density = np.exp(estimated_log_density)
        # Ensure density integrates to 1 over the grid
        estimated_density /= np.sum(estimated_density * self.delta_j)
        
        return self.grid_midpoints, estimated_density
    
    def get_results(self) -> Dict:
        """
        Get comprehensive, standardized results from the fitting process.
        """
        if not self.is_fitted:
        ...
        return {
            ...
            "data_points": self._grid_points_hal,
            "grid_points_hal_selected": self.grid_points_hal_selected,
            "n_selected_knots": len(self.grid_points_hal_selected) if self.grid_points_hal_selected is not None else 0,
            "estimated_density": density,
            "grid_points": grid_points,
            "intercept": self.theta_hat[0] if self.theta_hat is not None else None,
            "hal_coeffs": self.theta_hat[1:] if self.theta_hat is not None else None,
            # Add any other estimator-specific results for inspection
